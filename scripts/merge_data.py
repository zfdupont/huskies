...
def merge_NY(agg):
    ...
    gdf = gdf.rename(columns={'R_2020_pres': 'REPUBLICAN', 'D_2020_pres': 'DEMOCRAT','TOTAL_ADJ':'POPTOT', 'TOTAL_VAP_ADJ':'VAPTOTAL', 'WHITE_VAP_ADJ':'VAPWHITE', 'BLACK_VAP_ADJ':'VAPBLACK','AMIND_VAP_ADJ':'VAPNATIVE','ASIAN_VAP_ADJ':'VAPASIAN','HWN_VAP_ADJ':'VAPHWN','OTHER_VAP_ADJ':'VAPOTHER','MULTI_VAP_ADJ':'VAPMIXED','HISP_VAP_ADJ':'VAPHISP'})
    ...
    if agg:
        gdf = aggregate(gdf, 'district_id_21', "POPTOT  VAPTOTAL  VAPWHITE  VAPBLACK  VAPNATIVE  VAPASIAN  VAPHWN  VAPOTHER  VAPMIXED  VAPHISP  REPUBLICAN  DEMOCRAT".split())
        gdf.to_file('./genereated/NY/preprocess/mergedNYD.geojson', driver='GeoJSON')
def merge_GA(agg):
    ...
    gdf = merge_data(gdf, "GEOID20", [e_data, d_data])
    gdf = gdf.rename(columns={'G20PRERTRU': 'REPUBLICAN', 'G20PREDBID': 'DEMOCRAT', 'P0010001':'POPTOT','P0030001':'VAPTOTAL', 'P0030003':'VAPWHITE', 'P0030004':'VAPBLACK','P0030005':'VAPNATIVE','P0030006':'VAPASIAN','P0030007':'VAPHWN','P0030008':'VAPOTHER','P0030009':'VAPMIXED','P0040002':'VAPHISP'})
    gdf = gpd.GeoDataFrame(gdf, geometry='geometry')
    ...
    if agg:
        gdf = aggregate(gdf,'district_id_21',"POPTOT  VAPTOTAL  VAPWHITE  VAPBLACK  VAPNATIVE  VAPASIAN  VAPHWN  VAPOTHER  VAPMIXED  VAPHISP  REPUBLICAN  DEMOCRAT".split())
        gdf.to_file('./generated/GA/preprocess/mergedGAD.geojson', driver='GeoJSON')
def merge_IL(agg):
    ...
    if agg:
        gdf = aggregate(gdf,'district_id_21',"POPTOT  VAPTOTAL  VAPWHITE  VAPBLACK  VAPNATIVE  VAPASIAN  VAPHWN  VAPOTHER  VAPMIXED  VAPHISP  REPUBLICAN  DEMOCRAT".split())
        gdf.to_file('./generated/IL/preprocess/mergedILD.geojson', driver='GeoJSON')
def merge_all(agg):
    merge_NY(agg)
    merge_GA(agg)
    merge_IL(agg)
